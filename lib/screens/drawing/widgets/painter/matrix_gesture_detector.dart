...
class _MatrixGestureDetectorState extends State<MatrixGestureDetector> {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onScaleStart: widget.trackMatrix ? onScaleStart : null,
      onScaleUpdate: widget.trackMatrix ? onScaleUpdate : null,
      // onScaleEnd: onScaleEnd,
      onPanUpdate: !widget.trackMatrix ? widget.onPanUpdate : null,
      onPanStart: !widget.trackMatrix ? widget.onPanStart : null,
      onPanEnd: !widget.trackMatrix ? widget.onPanEnd : null,
      child: child,
    );
  }

  _ValueUpdater<Offset> translationUpdater = _ValueUpdater(
      onUpdate: (oldVal, newVal) => newVal - oldVal, value: Offset.zero);
  _ValueUpdater<double> rotationUpdater =
      _ValueUpdater(onUpdate: (oldVal, newVal) => newVal - oldVal, value: 0);
  _ValueUpdater<double> scaleUpdater =
      _ValueUpdater(onUpdate: (oldVal, newVal) => newVal / oldVal, value: 0);

  void onScaleStart(ScaleStartDetails details) {
    // if (!widget.trackMatrix) {
    //   widget.onPanStart(new DragStartDetails(
  ...
  void onScaleUpdate(ScaleUpdateDetails details) {
    // if (!widget.trackMatrix) {
    //   widget.onPanUpdate(new DragUpdateDetails(
    //       globalPosition: details.focalPoint,
    //       localPosition: details.localFocalPoint));
    //   return;
    // }
  ...
  void onScaleEnd(ScaleEndDetails details) {
    // if (!widget.trackMatrix) {
    //   widget.onPanEnd(new DragEndDetails(
...
class _ValueUpdater<T> {
  ...
  _ValueUpdater({required this.onUpdate, required this.value});

  T update(T newValue) {
    T updated = onUpdate.call(value, newValue);
    value = newValue;
    return updated;
  }
}
